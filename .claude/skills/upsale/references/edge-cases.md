...
## Step 4.1.09 — audit-security composition failures (`--high` only)
...
- Artifact is ALWAYS written when `--high` is set — no upstream BLOCK from 4.1.09 even on degradation.
- Aspect 4.2.06 reads `09-source-code-security.md` when present; when absent (default, no `--high`), it consumes only `06-security-compliance.md`.
