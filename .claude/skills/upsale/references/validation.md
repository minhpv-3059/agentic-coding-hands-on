...
As part of check 1 (and persisted regardless of decision), decompose BOTH the item's `**Need:**` bullet AND its `**Proposed solution:**` bullet into atomic claims and record ONE audit entry per claim. Each `**Clause:**` MUST start with `(need)` or `(solution)` — the tag identifies which bullet the claim came from. Audits cover those two bullets only — Value/Use-context/Benefits/Security/Formatting verdicts are summarised in `# Reason` by citing the check number, not added to `# Audits`. Each audit captures: `Clause` = the atomic claim verbatim/lightly-paraphrased from the source bullet (preserve every `path:line`, version, advisory ID, route, and metric exactly), prefixed with `(need)` or `(solution)`; `Evidence` = what was inspected to verify the claim, with the supporting **repo** `path:line` (NOT a `plans/upsale/**` path — see "Evidence source rule" below), or the search performed and its negative result when unsupportable; `Verdict` = `correct` when the cited evidence supports the claim, or `wrong` when the citation is stale/fabricated, the version/ID does not match the lockfile, the metric is absent, the proposed library/API/path/version is unsupportable on the detected stack, or the assertion contradicts the repo. A "claim" is an independently verifiable assertion — split each bullet on `;`, `.`, and conjunctions whenever each fragment carries an independently checkable assertion. A `(need)` claim verifies against backward-looking repo state (what already exists); a `(solution)` claim verifies against forward-looking stack-fit — a library/dep against lockfile or manifest, a file path against repo tree and folder conventions, an API symbol against the cited library version, a version pin against existing peer ranges, a pattern/refactor against repo grep for prior implementation, a config/env key against the env schema. A purely high-level solution (no specific library / path / API / version named) emits exactly ONE `(solution)` claim stating the high-level pattern, with evidence that the stack accepts (or rejects) the pattern — don't manufacture sub-claims. Always emit `# Audits`, including on DROP, on items with no `**Need:**` bullet (emit a single `(need) Need bullet absent or empty` clause with `wrong` verdict), and on items with no `**Proposed solution:**` bullet (emit a single `(solution) Proposed solution bullet absent or empty` clause with `wrong` verdict).

### Evidence source rule (HARD)

Audited evidence MUST cite a **real repo** path — source, lockfile, manifest, config, in-repo spec, test, doc, CI workflow. **NEVER cite a `plans/upsale/**` path as evidence.** Those files are this skill's own generated artifacts; accepting them as evidence is circular — a hallucinated claim could validate itself by pointing at the upstream subagent that fabricated it.

`item_evidence` IS loaded from `plans/upsale/**` — that is fine; it tells the validator *which* claims to check and *which repo files* they point at. To verify, open the repo file the claim references and confirm the cited line / version / advisory ID / route / metric exists. When a claim's only support is a `plans/upsale/**` path with no underlying repo pointer (typical for metrics and negative assertions), run a repo grep / file lookup for the asserted fact, cite the negative result, and mark the verdict `wrong`.

`# Reason` and `decision:` MAY reference `plans/upsale/**` for cross-process traceability. The ban applies ONLY to `**Evidence:**` lines under `# Audits`. The ban applies identically to `(need)` and `(solution)` evidence — `(solution)` evidence MUST cite lockfile / manifest / repo tree / config / library-docs (when available), never `plans/upsale/**`.
...
Read the title plus every bullet (`Value`, `Need`, `Benefits`, `Proposed solution`, `Effort hint`) together. Use `item_evidence` to learn *which* claims to check and *which repo paths* they point at, then verify each claim against the **actual repo** (see "Evidence source rule" above — `plans/upsale/**` is never authoritative for verdict evidence). Form one end-to-end judgment by asking:

- Does the **Need** point at a real, evidenced problem? Every `path:line`, file path, dependency version, spec ID, route, advisory ID, and metric must resolve against a **real repo file** (see Evidence source rule above). CVE/GHSA/RUSTSEC IDs must match the package/version in a lockfile/manifest.
- Does the **Proposed solution** actually address that Need? Is it implementable in the detected stack (technical) or consistent with spec vocabulary (business)? Each Proposed-solution claim (library, file path, API, version, pattern, config) must resolve against the real repo / stack and is audited under the `(solution)` tag — a claim that fails its lockfile / repo-tree / version-compat check is `wrong` and feeds into the REVISE/DROP synthesis below.
...
- **Clause:** <(need) or (solution) tag + atomic claim from the item's `**Need:**` or `**Proposed solution:**` bullet, preserving any `path:line` / version / advisory ID / route / metric verbatim>
  - **Evidence:** <what was inspected + a **repo** `path:line` citation (NEVER `plans/upsale/**` — see Evidence source rule), OR the search performed and its negative result when the claim is unsupportable>
  - **Verdict:** <correct | wrong>
<one bullet per atomic claim. Emit all `(need)` bullets first (Need's document order), then all `(solution)` bullets (Proposed solution's document order). Always emit `# Audits` (including on DROP and when Need/Proposed-solution is absent/empty — see template `templates/validation-item.md` §Audits block rules).>
