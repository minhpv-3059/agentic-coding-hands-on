...
- `plans/upsale/technical/01-discovery/06-security-compliance.md` (always present) — dependency CVEs, secret-management posture, supply-chain hygiene, license manifest, runtime EOL. Authoritative for the **deps** sub-domain.
- `plans/upsale/technical/01-discovery/09-source-code-security.md` (present only when `--high` was set) — STRIDE / OWASP Top 10 source-code findings produced by composing `tkm:audit-security` in `full` mode. Authoritative for the **SAST** sub-domain.
