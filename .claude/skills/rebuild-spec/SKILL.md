...
3.6. Wave 5.6 — FeatureList quality gate before W6 dispatch. 8 checks across 3 groups: (A) structural — US###/SCR### coverage completeness, orphan codes, F-code uniqueness; (B) quality — single intent per F###, clear flow, vague naming, scope overlap; (C) grouping coherence. Halts on critical. W6 dispatches only after `passed: true`. Skipped in incremental mode when W5 not re-run.
4. Wave 6 — fan-out `researcher` × F### extracted from FeatureList (parallel, `blockedBy` W5.6). If F### count > 20, split into **batches of 5** (env override: `REBUILD_W6_BATCH_SIZE`) (`Wave6.batch-01`, `Wave6.batch-02`, …) to bound peak context and rate-limit pressure. Each researcher removes its own `.pending` on success.
...
8.5. Wave 7.5 — `structural_fixer.py` inserts missing `**Linked FR:**` placeholders in BR/SM/ALG/INT blocks (edge-case safety net; W6.5 validator catches systematic omissions). **Wave 7.6 removed** — W6.5 `FeatureSpec.linked_fr_missing` gate eliminates the need for placeholder resolution.

**Review zone (W7a | W7b | W7-merge):** LLM semantic checks.  
**Pre-fix zone (W7.5):** Deterministic structural safety net.
9. Wave 8 — per-cycle fix fan-out: orchestrator extracts issues grouped by file from review-report.md, spawns one implementer per affected file in parallel (cap: `REBUILD_W8_MAX_PARALLEL`, default 5, each with explicit issue list). Single re-reviewer verifies all fixes. Repeats up to `MAX_FIX_CYCLES=3`; escalates to user if still failing.
10. Wave 9 — pre-flight gate reads `validation-summary.json` + review frontmatter; HALTS if `overall_status == FAIL` OR `failed > 0` OR `missing > 0`. On pass: `promote_drafts.py` handles all file-ops (cp drafts → docs/specs/, stub, archive, GC, sha256 manifest); **Wave 9.5 runs immediately after in the orchestrator context** (`build_source_to_fcode.py` — reverse-index refresh; runs before the flag so flag existence ↔ both W9 + W9.5 done); `doc-writer` writes completion flag + **self-closes via `TaskUpdate(status=completed)` before returning**; `system-overview.md` promoted as redirect stub per `claude/skills/_shared/docs-canonical-mapping.md` § Stub Rule.

**ScreenSpec:** Generated via standalone pass (`/rebuild-spec --screen-specs`), not embedded in main pipeline. Run after main rebuild completes.
...
| `--screen-specs` | Run standalone ScreenSpec pass. Requires `docs/specs/screen-list.md`. Incremental: regenerates only screens changed since last pass. |
| `--with-screen-specs` | _(deprecated — use `--screen-specs` standalone pass)_ |
...
| `behavior-logic` | W2 | screen-list.md + screen-flow.md |
| `permissions` | W3 | screen-list.md + behavior-logic.md |
...
| 5.6 | `reviewer` | `feature-list.md` + US### headers + SCR### index | `feature-list-review.md` (frontmatter: `passed`, `issues`, `warnings`); halts W6 on critical |
...
| SS.1 | `researcher` (batched 5/task; standalone pass) | docs/specs/screen-list.md + data-model.md + `screen-spec-template.md` + `screen-spec-researcher-contract.md` | `plans/<active>/artifacts/screens/SCR###_Name/spec.md` |
| SS.2 | `reviewer` (screen-spec; standalone pass) | ScreenSpec batches (5/reviewer) + `verification-checklist.md` § ScreenSpec | `screen-review-batch-NN.md` + `TaskUpdate(status=completed)` |
...
- **Feature specs (v2.3.0+, updated v3.3.0):** user-journey-first layout with business-reader preamble (`## Why This Exists` / `## Who Uses It` / `## Business Workflow` / `## Screen Flow`); FR/BR/SM/ALG/INT/SC codes inline under owning US or under `## Cross-Cutting Logic`; `## Artifact References` merged table (replaces legacy `## Related Artifacts` + `## Spec Documents` two-section format — CRITICAL immediately on legacy); `## Screen Flow` uses bullet list (replaces deprecated 3-column Screen Route Table — CRITICAL immediately on old table); see `templates/feature-spec-template.md`.
...
- `scripts/migrate-behavior-logic-rename.py` — idempotent migration: renames `background-logic.md` → `behavior-logic.md` in existing `docs/specs/` trees, updates internal links, and patches `doc_shas` key in `.rebuild-state.json` to prevent false OOB-detection miss on next incremental run
