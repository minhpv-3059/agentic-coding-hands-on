    ...
    if os.path.isdir(screens_src):
        if args.mode == "full":
            scr_dirs = sorted([
                ...
                and re.fullmatch(r"SCR\d{3,4}(?:_\w+)?", d)
            ])
        else:
            raw = [c.strip() for c in (args.affected_screens or "").split(",") if c.strip()]
            scr_dirs = []
            for code in raw:
                if not re.fullmatch(r"SCR\d{3,4}", code):
                    print(f"[WARN] invalid SCR code skipped: {code}", file=sys.stderr)
                    continue
                # Prefer exact `code_slug` match; fall back to bare `code`
                matches = [
                    d for d in os.listdir(screens_src)
                ...
                if len(matches) == 1:
                    ...
                    sys.exit(2)
                # zero matches → silently skip (draft may be absent if hydrated from prior run)
        ...
        for sdir in scr_dirs:
            ...
            if not os.path.isfile(spec_src):
                ...
                continue
            spec_dst = os.path.join(docs_specs, "screens", sdir, "spec.md")
            try:
