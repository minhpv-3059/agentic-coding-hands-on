...
# promote_drafts.py uses os.getcwd() as the security base and
# _read_stub_from_mapping resolves against the script's real path.
...
CORE_ARTIFACTS = [
    ...
    "behavior-logic.md", "permissions.md", "user-stories.md", "feature-list.md",
]
...
def _seed_hydrate_screen_env(tmp: Path, affected_screens: list[str]) -> tuple[Path, Path]:
    """Create plan + docs dirs for hydrate screen-spec test."""
    ...
    # Core artifacts in docs/specs/ (must all exist to avoid full-mode fallback)
    ...
    # Screen spec dirs in docs/specs/screens/
    for name in ["SCR001_Foo", "SCR002_Bar", "SCR003_Baz"]:
        d = docs_specs / "screens" / name
        d.mkdir(parents=True)
        num = name.split("_")[0][-1]
        (d / "spec.md").write_text(f"screen {num}\n")

    # .rebuild-state.json (minimal)
    (docs_specs / ".rebuild-state.json").write_text(json.dumps({"mode": "incremental"}))
...
class TestHydrateScreens:
    def test_skips_affected_copies_others(self, tmp_path):
        plan_dir, docs_specs = _seed_hydrate_screen_env(tmp_path, affected_screens=["SCR002"])
...
def _seed_planner_screen_env(tmp: Path) -> dict:
    """Seed a git repo with minimal planner requirements + screen-list.md.
    ...
    (docs_specs / "_canonical-fcodes.json").write_text(canonical_json)

    # Reverse index
    (docs_specs / "_source-to-fcode.json").write_text(json.dumps({"index": {}}))
    ...
    # screen-list.md with two SCR sections
    screen_list = (
        "# Screen List\n\n"
        "## SCR001_Login\nLogin screen content.\n\n"
        "## SCR002_Dashboard\nDashboard content.\n"
    )
    (docs_specs / "screen-list.md").write_text(screen_list)

    return {
        "plan_dir": str(plan_dir),
        "docs_specs": str(docs_specs),
        "out": str(artifacts / ".incremental-plan.json"),
        "screen_list_path": docs_specs / "screen-list.md",
    }
...
class TestPlannerPayload:
    def test_incremental_emits_affected_screens(self, tmp_path):
        ...
        parsed = _parse_screen_sections(Path(env_paths["docs_specs"]) / "screen-list.md")
        shas = _hash_screen_sections(parsed)

        # _init_git_repo does `git add .` + commit, capturing all env files in one shot
        initial_sha = _init_git_repo(tmp_path)

        # Write state (untracked at this point) pointing at the initial commit
        ...
        (Path(env_paths["docs_specs"]) / ".rebuild-state.json").write_text(json.dumps(state))
        ...
        # Modify SCR002 section in screen-list.md and commit both changes
        new_content = (
            "# Screen List\n\n"
            "## SCR001_Login\nLogin screen content.\n\n"
            "## SCR002_Dashboard\nDashboard MODIFIED content.\n"
        )
        (Path(env_paths["docs_specs"]) / "screen-list.md").write_text(new_content)
        ...
        r = subprocess.run(
            [PYTHON, str(PLANNER_SCRIPT),
             ...
             "--docs-specs", env_paths["docs_specs"],
             "--out", env_paths["out"]],
...
class TestPromoteScreens:
    def test_incremental_promotes_only_listed(self):
        plan_dir, docs_specs = _seed_promote_screen_env(
        ...
        # SCR003 in affected-screens → should be promoted
        assert (docs_specs / "screens" / "SCR003_Baz" / "spec.md").exists()
        # SCR001 NOT in affected-screens → should NOT be promoted
        assert not (docs_specs / "screens" / "SCR001_Foo" / "spec.md").exists()
    ...
    def test_full_promotes_all(self):
        ...
        assert (docs_specs / "screens" / "SCR001_Foo" / "spec.md").exists()
        assert (docs_specs / "screens" / "SCR003_Baz" / "spec.md").exists()
