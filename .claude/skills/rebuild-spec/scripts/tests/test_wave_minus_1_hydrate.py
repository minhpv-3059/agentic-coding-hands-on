...
def _seed_screen_dirs(docs_specs: Path) -> None:
    """Seed docs/specs/screens/ with two SCR### subdirectories containing spec.md."""
...
class TestHydrateScreenSpecs:
    def test_copies_non_affected_screen_specs(self, tmp_path):
        plan_dir, docs_specs = _seed_hydrate_env(tmp_path, [], [])
        _seed_screen_dirs(docs_specs)
        # affected_screens present: SCR001 is affected, SCR002 is not
        plan_data = json.loads((plan_dir / "artifacts" / ".incremental-plan.json").read_text())
    ...
    def test_no_copy_when_affected_screens_absent(self, tmp_path):
        ...
        plan_dir, docs_specs = _seed_hydrate_env(tmp_path, [], [])
        _seed_screen_dirs(docs_specs)
        # Do NOT add affected_screens key → simulates full-mode payload shape
        _hydrate(plan_dir, docs_specs)
