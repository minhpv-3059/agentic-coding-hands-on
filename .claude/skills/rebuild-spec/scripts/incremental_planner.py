...
def _parse_screen_sections(screen_list_path: Path) -> dict[str, tuple[str, str]]:
    ...
    heading_re = re.compile(r"^## (SCR\d{3,4})(?:_(\w+))?")
...
def _hydrate(plan_dir: Path, docs_specs: Path) -> int:
    ...
    screens_src = docs_specs / "screens"
    ...
    if screens_src.is_dir() and "affected_screens" in plan_data:
        ...
        for sdir in sorted(screens_src.iterdir()):
            if not sdir.is_dir():
                continue
            m = re.match(r"^(SCR\d{3,4})", sdir.name)
            if not m:
                continue
            code = m.group(1)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.full:
        # W2.5: compute screen sha snapshot from docs/specs/screen-list.md if present
        _sl_path = docs_specs / "screen-list.md"
    ...
    _sl_path_inc = docs_specs / "screen-list.md"
    if not _sl_path_inc.is_file():
        _sl_path_inc = plan_dir / "artifacts" / "screen-list.md"
        if _sl_path_inc.is_file():
            print("[WARN] docs/specs/screen-list.md absent; using artifacts copy for screen sha diff", file=sys.stderr)
