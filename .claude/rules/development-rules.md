...
- Use `reviewer` agent to review code after every implementation
...
- **[AUTO-COMMIT RULE]** After emitting the Delivery Manifest (all boxes checked), immediately delegate to `git-manager` to commit — do NOT wait for the user to confirm. The Delivery Manifest itself is the user's seal. Only push to remote if the user explicitly requests it.
...
## Visual Aids
- Use `/tkm:preview-output --explain` when explaining unfamiliar code patterns or complex logic
- Use `/tkm:preview-output --diagram` for architecture diagrams and data flow visualization
- Use `/tkm:preview-output --slides` for step-by-step walkthroughs and presentations
- Use `/tkm:preview-output --ascii` for terminal-friendly diagrams (no browser needed to understand)
